...
static uint16_t SOOL_TimerInputCapture_GetSavedCounterVal(volatile SOOL_TimerInputCapture *tim_ic_ptr);
static uint8_t SOOL_TimerInputCapture_InterruptHandler(volatile SOOL_TimerInputCapture *tim_ic_ptr);

// helper
static uint16_t SOOL_TimerInputCapture_GetCaptureRegisterValue(volatile SOOL_TimerInputCapture *tim_ic_ptr);
 ...
 * @param period: Specifies the period value to be loaded into the active Auto-Reload Register at the next update event.
...
volatile SOOL_TimerInputCapture SOOL_Periph_TIM_TimerInputCapture_Init(TIM_TypeDef* TIMx,
	...
	volatile SOOL_TimerBasic tim_basic = SOOL_Periph_TIM_TimerBasic_Init(TIMx, prescaler, period);
...
static void SOOL_TimerInputCapture_Start(volatile SOOL_TimerInputCapture *tim_ic_ptr) {
	...
	tim_ic_ptr->tim_basic.Start(&tim_ic_ptr->tim_basic);
...
static void SOOL_TimerInputCapture_Stop(volatile SOOL_TimerInputCapture *tim_ic_ptr) {
	...
	tim_ic_ptr->tim_basic.Stop(&tim_ic_ptr->tim_basic);
...
static uint8_t SOOL_TimerInputCapture_InterruptHandler(volatile SOOL_TimerInputCapture *tim_ic_ptr) {
	...
	if (TIM_GetITStatus(tim_ic_ptr->tim_basic._TIMx, tim_ic_ptr->_setup.TIM_IT_CCx) == RESET) {
	...
	tim_ic_ptr->tim_basic._TIMx->SR = (uint16_t)~tim_ic_ptr->_setup.TIM_FLAG_CCxOF;

	/* Clear IT pending bit */
	tim_ic_ptr->tim_basic._TIMx->SR = (uint16_t)~tim_ic_ptr->_setup.TIM_IT_CCx; // TIM_ClearITPendingBit(timer->_TIMx, TIM_IT_Update);
	return (1);
